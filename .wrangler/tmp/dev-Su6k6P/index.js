...
// .wrangler/tmp/bundle-WYZDrN/checked-fetch.js
...
var require_performance_now = __commonJS({
  "node_modules/performance-now/lib/performance-now.js"(exports, module) {
    ...
    init_virtual_unenv_global_polyfill_clear_immediate();
    (function() {
      var getNanoSeconds, hrtime3, loadTime, moduleLoadTime, nodeLoadTime, upTime;
...
// .wrangler/tmp/bundle-WYZDrN/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
// .wrangler/tmp/bundle-WYZDrN/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_process();
init_virtual_unenv_global_polyfill_performance();
...
var WSAETOOMANYREFS = 10059;
var WSAETIMEDOUT = 10060;
var WSAECONNREFUSED = 10061;
var WSAELOOP = 10062;
var WSAENAMETOOLONG = 10063;
var WSAEHOSTDOWN = 10064;
var WSAEHOSTUNREACH = 10065;
var WSAENOTEMPTY = 10066;
var WSAEPROCLIM = 10067;
var WSAEUSERS = 10068;
var WSAEDQUOT = 10069;
var WSAESTALE = 10070;
var WSAEREMOTE = 10071;
var WSASYSNOTREADY = 10091;
var WSAVERNOTSUPPORTED = 10092;
var WSANOTINITIALISED = 10093;
var WSAEDISCON = 10101;
var WSAENOMORE = 10102;
var WSAECANCELLED = 10103;
var WSAEINVALIDPROCTABLE = 10104;
var WSAEINVALIDPROVIDER = 10105;
var WSAEPROVIDERFAILEDINIT = 10106;
var WSASYSCALLFAILURE = 10107;
var WSASERVICE_NOT_FOUND = 10108;
var WSATYPE_NOT_FOUND = 100109;
var WSA_E_NO_MORE = 10110;
var WSA_E_CANCELLED = 10111;
var WSAEREFUSED = 10112;
var SIGHUP = 1;
var SIGINT = 2;
var SIGQUIT = 3;
var SIGILL = 4;
...
var app = new Hono2();
app.all("*", async (c) => {
  c.env.ASSETS.fetch(c.req.url);
});
app.use("/api/db", bearerAuth({ token: apikey_default.apikey_list.map((apikey) => apikey.key) }));
app.get("/api/db", async (c) => {
  const apiKey = c.req.header("Authorization")?.split(" ")[1];
  const table3 = apikey_default.apikey_list.find((apikey) => apikey.key === apiKey)?.table;
  var tableExists;
  try {
    tableExists = await c.env.DB.prepare(`SELECT * FROM '${table3}'`).all();
  } catch (e) {
    console.warn(e.message);
    await c.env.DB.prepare(`CREATE TABLE IF NOT EXISTS ${table3} (id INTEGER PRIMARY KEY, data TEXT)`).all();
  }
...
app.post("/api/db", async (c) => {
  ...
  try {
    await c.env.DB.prepare(`INSERT INTO ${table3} (data) VALUES (?) ON CONFLICT (id) DO UPDATE SET data = ?`).bind([data, data]).run();
  } catch (e) {
...
app.delete("/api/db", async (c) => {
  ...
  try {
    await c.env.DB.prepare(`DELETE FROM ${table3} WHERE id = ?`).bind([id]).run();
  } catch (e) {
    return c.json({ success: false, message: e.message });
  }
...
app.get("/api/status", async (c) => {
  ...
  let listcpus = os_default2.cpus();
  let totalmem2 = bytesToSize(os_default2.totalmem());
  let freemem2 = bytesToSize(os_default2.freemem());
  let uptime3 = format(process.uptime());
  let hostname2 = os_default2.hostname();
  let platform3 = os_default2.platform();
  let speeds = (0, import_performance_now.default)();
  return c.json({
    success: true,
    hostname: hostname2,
    system_speed: (speeds / 60).toFixed(2) + " ms",
    free_memory: freemem2,
    total_memory: totalmem2,
    runtime: uptime3,
    platform: platform3,
    cpus: [...listcpus]
  });
